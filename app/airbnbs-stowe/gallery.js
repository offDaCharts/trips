...
document.querySelectorAll('.shot').forEach(button=>button.addEventListener('click',()=>{opener=button;currentStay=window.STOWE_STAYS.find(s=>s.key===button.dataset.stay);currentIndex=Number(button.dataset.index);showPhoto();gallery.showModal();}));
